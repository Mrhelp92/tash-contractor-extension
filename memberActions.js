...
function runMemberFlow() {
    ...
    waitForElement("input.search-input", (input) => {
        input.focus();
        input.click();
        ...
        .then(data => {
            ...
            typeTextSlowly(input, contractorEmail, () => {
                ...
                waitForElement(".cu-dropdown__left > .cu-dropdown__toggle", (dropdownBtn) => {
                    dropdownBtn.click();
                    console.log("🧩 Clicked dropdown toggle");

                    // Step 2: Wait for the menu items
                    waitForElement("div.cu-dropdown-list-item__link_cancelinvite, div.cu-dropdown-list-item__link_removemember", () => {
                        const item2 = document.querySelector("div.cu-dropdown-list-item__link_cancelinvite");
                        const item3 = document.querySelector("div.cu-dropdown-list-item__link_removemember");

                        const item2Text = item2?.innerText?.trim().toLowerCase() || "";
                        const item3Text = item3?.innerText?.trim().toLowerCase() || "";

                        console.log("🧪 item2 exists?", !!item2);
                        console.log("🧪 item3 exists?", !!item3);
